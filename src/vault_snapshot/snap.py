...
from vault_snapshot.snap import take_snapshot
...
def parse_args():
    ...
    parser.add_argument(
        "--s3", action="store_true", help="indicates to save snapshot to s3"
    )
...
def parse_env():
    ...
    s3_bucket = os.environ.get("VAULT_SNAPSHOT_S3_BUCKET", "")
    s3_prefix = os.environ.get("VAULT_SNAPSHOT_S3_PREFIX", "dev")
    # local dir to store snapshot
    local_dir = os.environ.get("LOCAL_DIR", "/tmp/vault_snapshot")
    return (local_dir, vault_host, vault_port, vault_role, s3_bucket, s3_prefix)
...
if __name__ == "__main__":
    ...
    args = parse_args()
    loglevel, artsy_env, s3 = (args.loglevel, args.artsy_env, args.s3)

    setup_logging(eval("logging." + loglevel))

    (local_dir, vault_host, vault_port, vault_role, s3_bucket, s3_prefix) = parse_env()

    validate(artsy_env, vault_host, vault_port, s3, s3_bucket)
